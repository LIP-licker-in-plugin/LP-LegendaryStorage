package com.licker2689.lls.functions;
...
public class LLSFunction {
    ...
    public static void updateCurrentPage(LInventory inv) {
    ...
    public static void openStorage(Player p, Player target) {
        ...
        LInventory inv = new LInventory(null, ColorUtils.applyColor(title), 54, true, plugin);
        inv.setPages(0);
        inv.setObj(target.getUniqueId());

        ItemStack pane = NBT.setStringTag(new ItemStack(Material.STAINED_GLASS_PANE, 1, (short) 7), "page", "true");
        ItemStack block = NBT.setStringTag(new ItemStack(Material.BARRIER), "lls-block", "true");
        ItemStack prev = NBT.setStringTag(new ItemStack(Material.INK_SACK, 1, (short) 9), "prev", "true");
        ...
        prev.setItemMeta(im);
        ItemStack next = NBT.setStringTag(new ItemStack(Material.INK_SACK, 1, (short) 10), "next", "true");
        im = next.getItemMeta();
    ...
    public static void saveCurrentContents(LInventory inv) {
    ...
    public static void saveInventory(LInventory inv) {
        ...
        for (ItemStack[] item : inv.getPageItems().values()) {
            for (int i = 0; i < item.length; i++) {
                if (!NBT.hasTagKey(item[i], "lls-block")) {
    ...
    public static void getCoupon(Player p, String sint) {
        ...
        p.getInventory().addItem(NBT.setIntTag(coupon, "lls-coupon", i));
    ...
    public static void useCoupon(Player p, ItemStack item) {
        int i = NBT.getIntegerTag(item, "lls-coupon");
        YamlConfiguration data = plugin.udata.get(p.getUniqueId());
        int owned = data.getInt("Owned");
